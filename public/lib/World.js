  ...
  this.World = (function() {
    ...
    World.prototype.createSprites = function(layerSprite, spatialHash) {
      ...
      doodadsSpriteFrameFunction = function(x, y, tileMap) {
        var grassCount, i, mt, offset, ret, tree, treeCount, _m, _n, _o, _p, _q;
        geographicTile = _this.geographicTileMap[x][y];
        ...
        if (geographicTile.elevation > 0) {
          if (geographicTile.elevation <= 2) {
            ...
            if (rainfall === "Wet" || rainfall === "Moist") {
              for (i = _p = 0; 0 <= grassCount ? _p < grassCount : _p > grassCount; i = 0 <= grassCount ? ++_p : --_p) {
                offset = randomOffset(grassTuft, _this.tileSize);
                ret.push({
                  spriteFrame: grassTuft,
                  x: offset.x,
              ...
              for (i = _q = 0; 0 <= treeCount ? _q < treeCount : _q > treeCount; i = 0 <= treeCount ? ++_q : --_q) {
                offset = randomOffset(tree, _this.tileSize);
                ret.push({
                  spriteFrame: tree,
                  x: offset.x,
